...
| `docs/.orch-state.json` | orchestrator 전용 | 세션 단위 | 현황(status·current_step·phase·updated_at)·사이클번호·dispatch카운트·예산·실패시그니처·열린결정 |
...
`status`·`current_step`·`phase`·`updated_at`은 사람·대시보드가 "지금 어디까지 됐나"를 추론 없이 읽도록 orchestrator가 유지하는 필드다. 이전 스키마는 `cycle`/`dispatches`만 있어 "현재 단계"를 PROCESS.md 자유 텍스트로만 알 수 있었다 → 이 필드들이 그 갭을 메운다.

| 필드 | 값 | 누가 언제 쓰나 |
|---|---|---|
| `status` | `not-started`·`running`·`awaiting-human`·`budget-exceeded`·`done` | 상태 전이 시 orchestrator |
| `current_step` | 활성 에이전트명(11개 중) 또는 `null` | **매 dispatch 직전** orchestrator |
| `phase` | DoD 게이트 단계 `0`(비DoD)~`7` | DoD 진입·진행 중 orchestrator |
| `updated_at` | ISO8601, 마지막 상태 변경 시각 | **상태를 바꿀 때마다** orchestrator (신선도 마커) |

- **공개 경계**: 위 4개 + `cycle`/`max_cycles`/`dispatches`/`max_dispatches`/`open_decisions`/`started_at`만 대시보드에 push 가능한 현황 manifest를 이룬다. `budget` 내부값을 넘는 정보·`failure_signatures`(에러 시그니처)는 **내부 전용 — push 금지**.
- **프로젝트 식별자**: `project_id`/`project_name`은 여기 두지 않는다. 옵트인 설정 `.claude/dashboard.json`(§11)에서 와서 push 시점에 합쳐진다. 따라서 `.orch-state.json`은 프로젝트 무관 — 토이 프로젝트도 이 파일을 정상 갱신하되 식별자·전송정보가 없어 아무것도 나가지 않는다.
...
## 11. 대시보드 보고 (옵트인) — 기본 비공개

프로젝트 현황·산출물을 비공개 어드민 대시보드로 보낼 수 있다. **기본값은 비공개(off)** — 토이/개인 프로젝트는 아무 설정도 안 하면 오늘과 100% 동일하게 동작하고 아무것도 밖으로 나가지 않는다.

### 스위치 = `.claude/dashboard.json`의 존재

대시보드 연결은 **오직 이 파일이 있고 유효 필드가 채워졌을 때만** 활성화된다(별도 커맨드·설치 플래그 없음 — 파일을 직접 만든다).

```jsonc
// .claude/dashboard.json   (★ git 커밋 금지 — .gitignore에 등재됨)
{
  "enabled": true,
  "project_id": "acme-billing",        // 어드민 등록 시 받은 id (멀티테넌시 키)
  "project_name": "ACME 빌링 개편",
  "ingest_url": "https://dashboard.example.com/api/ingest",
  "token": "prj_live_xxxxxxxx"          // 어드민이 발급한 비밀 토큰
}
```

- **끄기**: 파일 삭제 또는 `"enabled": false`(토큰 보존).
- 이 파일은 토큰을 담으므로 `.gitignore`에 등재한다(install.sh가 보장). 토이 프로젝트엔 파일 자체가 없다.

### 이중 안전장치 (fail-safe)

1. **프로젝트 쪽** — `.claude/dashboard.json`이 없거나 `enabled:false`면 reporter는 **아예 실행 안 됨**(no-op). 실수로라도 push 안 함.
2. **서버 쪽** — ingest는 **사전 등록된 `project_id` + 유효 토큰**만 수락. 등록 안 된 프로젝트는 받아도 거부.

→ 노출되려면 **옵트인(파일) + 등록(서버)** 양쪽이 모두 필요.

### 보고 시점 (reporter)

> 수집/ingest API·DB·어드민 구현은 별도 서비스(이 템플릿 밖). 여기서는 **프로젝트 쪽이 언제 무엇을 push하는가**의 프로토콜만 규정한다.

- 활성화돼 있으면, orchestrator는 **DoD 통과 후 커밋 직후(§5 step 7)**에 현황 manifest(§8 '대시보드 현황 필드' + 식별자) + 이번 사이클에 바뀐 산출물 md를 `ingest_url`로 `token`과 함께 POST한다.
- 비활성/미설정이면 이 단계를 **건너뛴다**.
- push 페이로드는 §8 공개 경계를 따른다 — `failure_signatures`·`budget` 내부값은 보내지 않는다.

### Ingest 페이로드 계약 (두 리포의 단일 인터페이스)

프로젝트 쪽(push)과 대시보드 서비스(ingest API)가 **이 형태로 정확히 일치**해야 한다. 바뀌면 양쪽을 함께 고친다.
...
```json
{
  ...
  "updated_at": "2026-06-09T08:00:00Z",
  "artifacts": [
    { "path": "specs/003-foo.md", "kind": "spec", "md": "# ..." }
  ]
}
```

- `token`은 `.claude/dashboard.json`의 토큰. 서버는 해시 대조 + `project_id` 등록 여부로 이중 검증.
- 서버는 수신 시각(`received_at`)을 **자기 시계로** 기록한다(신선도 판정 기준). 본문의 `updated_at`은 "프로젝트가 보고한 시각"으로 표시용.
- `artifacts`는 이번 사이클에 바뀐 md만. `kind`는 `plan|spec|process|decision`.
