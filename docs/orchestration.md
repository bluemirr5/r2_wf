...
| 판정 | 조건 | 처리 |
|---|---|---|
| **e2e 가능** | 표면 선언 + 하니스(`e2e:up`) 존재 + 기동 성공 | 실제 엔드포인트/UI/CLI 테스트 |
| **N/A(표면없음)** | 표면 미선언(순수 라이브러리/내부 로직) | **결정 스토어에 한 줄 기록 + DoD에서 플래그**(아래) |
| **N/A(하니스없음)** | 표면 있으나 하니스 미정의 | 사람에게 하니스 요청 보고 |
| **실패** | 하니스 기동 후 red | (a)프로덕션버그/(b)테스트오류/(c)환경문제 분류 → debugger |
...
**provisioning 경계**: e2e-tester는 **스스로 인프라를 세우지 않는다**(report-only 위반·시스템 상태 변경). 프로젝트가 제공하는 하니스 명령(`pnpm e2e:up`/`e2e:down`)을 **호출만** 한다. 하니스 자체는 프로젝트(developer/사람) 책임.
