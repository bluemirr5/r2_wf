...
2. **상태 로드**: `docs/.orch-state.json`을 `Read` (없으면 `docs/orch-state.example.json`을 복사해 초기화). `--budget` 인자가 있으면 `budget` 반영. 첫 실행이면 `status`를 `running`으로, `started_at`·`updated_at`을 현재 시각으로 기록 (현황 필드: [[orchestration]] §8)
...
**현황 필드 갱신(매 dispatch, [[orchestration]] §8)**: 에이전트를 dispatch하기 **직전** `current_step`을 그 에이전트명으로, `updated_at`을 현재 시각으로 기록한다. DoD 단계에 진입하면 `phase`(1~7)도 갱신. 이 필드들이 대시보드/사람이 "지금 어디"를 추론 없이 읽는 근거다.
...
5. **글로벌 예산 초과**(`dispatches`/`cycles`/`wall_clock`) → `status`를 `budget-exceeded`로·`updated_at` 갱신 → 즉시 정지 + §5 escalation

### 5. Escalation (사람 필요)

1. 결정 레코드가 없으면 생성(`docs/decisions/NNN-*.md`, §3 스키마, `created`·`deadline` 포함)
2. **notifier dispatch** → PushNotification(+설정시 Slack) 발송
3. `.orch-state.json`의 `open_decisions`에 id 추가, `status`를 `awaiting-human`으로·`updated_at` 갱신, PROCESS.md에 한 줄 포인터
...
8. **대시보드 보고(옵트인, [[orchestration]] §11)**: `.claude/dashboard.json`이 있고 `enabled:true`면 커밋 직후 현황 manifest + 이번 사이클 변경 산출물 md를 `ingest_url`로 push. **파일 없거나 `enabled:false`면 건너뜀**(토이 프로젝트 기본). `failure_signatures`·`budget` 내부값은 보내지 않음(§8 공개 경계)

각 DoD 단계 진입 시 `.orch-state.json`의 `phase`(1~7)·`updated_at`을 갱신한다.
...
게이트 통과 시 `.orch-state.json`: `cycle`+1, `phase`를 0으로, `failure_signatures` 리셋(사이클 단위), `updated_at` 갱신. **모든 사이클 소진 시** `status`를 `done`으로.
...
- **옵트인 안 된 프로젝트 push**: `.claude/dashboard.json` 없거나 `enabled:false`면 어떤 데이터도 밖으로 보내지 않음. `failure_signatures`·`budget` 내부값은 활성 상태여도 push 금지 ([[orchestration]] §8·§11)
