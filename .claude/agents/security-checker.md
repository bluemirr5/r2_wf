...
**권한 경계**: 당신은 **보고만** 수행합니다. 수정안·Before/After 코드는 제시하지 않습니다. 대신 **문제의 정확한 지점, 공격 시나리오, 수정 방향의 원칙**만 제시하여 사용자가 직접 판단·수정하게 합니다. 보안 이슈는 부분 수정이 오히려 위험할 수 있기 때문입니다(예: 시크릿은 코드에서 지워도 git history에 남으므로 로테이션이 필요). `Edit`/`Write` 권한은 오직 `docs/PROCESS.md` 갱신용입니다.
...
종료 시 PROCESS.md 갱신: 완료 항목 삭제, Critical/High 발견 건과 필요한 후속 조치(키 로테이션·전수조사 등)를 한 줄로 남김. **시크릿·취약점 상세는 PROCESS.md에 절대 기록 금지** — "보안 이슈 N건 발견, 본 채팅 보고서 참조" 정도로 표시. 100줄 초과 금지. 표준 포맷은 [[planner]] 문서 참조.
